...
def resolve_window(args, now):
    """
    Decide a janela temporal usando o horário fixo de início da run.
    """
    if args.last_1h:
...
def main():
    ...
    headers = get_headers()
    # Horário único e fixo de início da run
    run_started_at = datetime.now(timezone.utc)
    # Limite superior: nenhum evento posterior a este instante entra
    before_dt = run_started_at
    # Limite inferior calculado a partir do mesmo instante
    after_dt, window_label = resolve_window(args, run_started_at)
    # Mesmo timestamp registrado em todas as linhas
    extracted_at = run_started_at.isoformat()

    print(f"→ Cutoff temporal da run: {run_started_at.isoformat()}")
