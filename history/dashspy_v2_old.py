    ...
    for account_id in targets:
        ...
        if not completed:
            log.warning(
                ...
                "Use 'python dashspy_v2.py meta-resume' para retomar.",
                account_id,
            )
            break
...
def main() -> None:
    ...
    log.info("Iniciando dashspy_v2 — registro em: %s", recording_ts)
    ...
    if falhas:
        log.warning("dashspy_v2 finalizado com falhas no envio: %s", ", ".join(falhas.keys()))
    else:
        log.info("dashspy_v2 finalizado com sucesso.")
...
if __name__ == "__main__":
    ...
    if _cmd is None:
        ...
        _output_dir = _Path("PATH_OUTPUTS_M")
        print("\nContas Meta Ads disponíveis:")
        for _i, _acc in enumerate(META_AD_ACCOUNT_IDS, 1):
            print(f"  [{_i}] {_acc}")
